...
int main()
{
	...
	while(gameOver==0)
	{
		...
		do
		{
			if(playerTurn)
			{
				cout<<endl<<"Player Go"<<endl<<"Choose a Tile: ";
				cin>>selection;
				endTurn=board.tileChoose(playerTurn, selection);
				if (endTurn==0)
				{
					cout<<endl<<"Error"<<endl<<"Please Choose Again";
				}
				else if (endTurn)
				{
...
bool Board::gameCheck(bool playerTurn)
{
	...
	if (endGame==1)
	{
		...
		if(playerTurn)
		{
			cout<<endl<<"Congratulations You are not a loser";
			return 1;
		}
		else
		{
			cout<<endl<<"Sorry you're a loser";
			return 1;
		}
